...
for j = 2:n,                             % Look for fixed y(j) 
   ...
   for i=2:m,
      b(i-1) = kappa*(u1(i,j-1) -2*u1(i,j) + u1(i,j+1))/h1 + ...
		f1(t2,x(i),y(j)) + 2*u1(i,j)/dt;
      if i == 2
...
for i = 2:m,
   ...
   A = sparse(m-1,m-1); b=zeros(m-1,1);
   for j=2:n,
      b(j-1) = kappa*(u2(i-1,j) -2*u2(i,j) + u2(i+1,j))/h1 + ...
                f1(t2,x(i),y(j)) + 2*u2(i,j)/dt;
      if j == 2

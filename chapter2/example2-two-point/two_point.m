...
%    boundary value problem: -u''(x)+u'+u = f(x) using the center difference %
%    scheme.                                                           %  
%                                                                      %
%    Input:                                                            %
%     a, b: Two end points.                                            %
...
for i=1:n-2,
 % A(i,i) = -2/h1; A(i+1,i) = 1/h1; A(i,i+1)= 1/h1;
  A(i,i) = 2/h1+1.0; A(i+1,i) = -1/h1-1/(2.0*h); A(i,i+1)= -1/h1+1/(2.0*h);
end
  A(n-1,n-1) = 2/h1+1.0;
...
for i=1:n-1,
  ...
  F(1) = F(1) + ua/h1+ua/(2.0*h);
  F(n-1) = F(n-1) + ub/h1-ub/(2.0*h);
